...
2. **§5.2.3 measures the cause directly** — every intersection column KS 0.89–1.00; `vol_total` differs
   ~1,859× in raw scale — measured on raw data, independent of model behaviour.
...
4. **One partial exception reinforces this.** CNN's B→A cell: F1=0.0000 but ROC-AUC=0.773 — an overfit
   ...
   fixed 0.5 threshold, calibrated on B's scale, failed to land sensibly on A.
...
regimes sharing a conceptual taxonomy, not stand-ins for "any DNS exfiltration data."

## 8.2.5 What's still open

Proposed remedies (`log1p` on F1 before scaling, jointly-fit scaler) documented but not tested against
this matrix. Diagnosis covers `mode="intersection"` only; B_ONLY families (F4/F5) can't be tested for
